...
import { createUnifiedGraph, UnifiedGraph } from './unified-graph'
import { MCPToolDef } from './bridge-types'

// ─── Tool Definitions ─────────────────────────────────────────────────────────

export const GRAPH_MCP_TOOLS: MCPToolDef[] = [
  {
...
export class HermesGraphGateway {
  ...
  handleToolCall(toolName: string, args: Record<string, any>): string {
    if (!this.unified) return JSON.stringify({ error: 'Graph not initialized — run npx toongine init' })
    ...
    switch (toolName) {
      ...
      case 'toon_graph_callers': {
        ...
        if (matches.length === 0) return JSON.stringify({ error: `Symbol not found: ${symbol}` })
...
export function createGraphGateway(projectRoot: string): HermesGraphGateway {
